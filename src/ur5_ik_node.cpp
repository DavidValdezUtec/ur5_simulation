...
std::string ur5_pos = get_file_path("ur5_simulation",     "launch/output_data2.txt");
...
Eigen::Vector3d forwardKinematics(const pinocchio::Model& model, 
                                  pinocchio::Data& data, 
...
Eigen::MatrixXd jacobian(const pinocchio::Model& model, 
                         pinocchio::Data& data, 
                         const pinocchio::FrameIndex& tool_frame_id, 
                         const Eigen::VectorXd& q) {
...
Eigen::VectorXd computeQP_IK(const Eigen::MatrixXd& J, 
                             const Eigen::VectorXd& x_error) {
...
Eigen::VectorXd computeQP_IK_WeightedError(const Eigen::MatrixXd& J,
                                         const Eigen::VectorXd& x_error,
...
class UR5eJointController : public rclcpp::Node {
    public:
        UR5eJointController() : Node("ur5e_joint_controller"), time_elapsed_(0.0) {
            ...
            output_file_2.open(ur5_pos, std::ios::out | std::ios::app);
            ...
            subscription_ = this->create_subscription<sensor_msgs::msg::JointState>(
                "/joint_states", 10, std::bind(&UR5eJointController::update_joint_positions, this, std::placeholders::_1));
            
                // Suscriptor para leer la posición del haptic phantom
            subscription_haptic_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
                "/phantom/pose", 10, std::bind(&UR5eJointController::pose_callback, this, std::placeholders::_1)
            ); 
            // Temporizador para calcular y publicar nuevas posiciones articulares
            timer2_ = this->create_wall_timer(
                std::chrono::seconds(1), // Frecuencia de 10 Hz
                std::bind(&UR5eJointController::posicion_inicial, this));

            timer_ = this->create_wall_timer(
                std::chrono::milliseconds(control_loop_time), // Frecuencia de 10 Hz
                std::bind(&UR5eJointController::control_loop, this));
    
            // Inicializar posiciones articulares
    ...
    private:
        ...
        void pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
            ...
            //lee el quaternion del haptico y lo convierte a matriz de rotacion
            ...
            Eigen::Vector3d euler_angles = extractEulerAngles(quaternion);

            q_x.w() =  std::cos(euler_angles[0] / 4); q_x.x() =  std::sin(euler_angles[0] / 4);q_x.y() =  0; q_x.z() =  0;
            ...
            /* cout<<"q_x: "<< q_x.w()<<" "<<q_x.x()<<" "<<q_x.y()<<" "<<q_x.z()<<endl;
            cout<<"q_y: "<< q_y.w()<<" "<<q_y.x()<<" "<<q_y.y()<<" "<<q_y.z()<<endl;
            cout<<"q_z: "<< q_z.w()<<" "<<q_z.x()<<" "<<q_z.y()<<" "<<q_z.z()<<endl; */

            // Imprimir los ángulos de Euler
            /*
            std::cout << "Roll (X): " << euler_angles[0] << " rad\n";
            std::cout << "Pitch (Y): " << euler_angles[1] << " rad\n";
            std::cout << "Yaw (Z): " << euler_angles[2] << " rad\n";
            */
            /*
            cout<<"Rotacion: "<<rotation_matrix(0,0)<<" "<<rotation_matrix(0,1)<<" "<<rotation_matrix(0,2)<<endl;
            cout<<"Rotacion: "<<rotation_matrix(1,0)<<" "<<rotation_matrix(1,1)<<" "<<rotation_matrix(1,2)<<endl;   
            cout<<"Rotacion: "<<rotation_matrix(2,0)<<" "<<rotation_matrix(2,1)<<" "<<rotation_matrix(2,2)<<endl;
            */
        }    
        ...
        /* void posicion_inicial() {
            ...
            point.positions = {q_init[0], q_init[1], q_init[2], q_init[3], q_init[4], q_[5]};
        ...
        void posicion_inicial() {
            try {
                load_values_from_file(config_path, q_init, 6, 7);       // Leer la 1ra línea para q_init            
            } catch (const std::exception &e) {
        ...
        void control_loop() {
            ...
            try {
                load_values_from_file(config_path, qt_init, 4, 8);       // Leer la 4ta línea para qt_init            
            } catch (const std::exception &e) {
                cerr << "Error al cargar el archivo de configuración: " << e.what() << endl;                
            }
            ...
            if (orientacion) {
                current_orientation = quat_initial;//* q_x * q_y * q_z;
            ...
            //double rot_des[4] = {1,0,0,0}; // (w,x,y,z)
            /* error_[0] = xd[0] - previous_error_[0];
            error_[1] = xd[1] - previous_error_[1];
            error_[2] = xd[2] - previous_error_[2];
          
     */
            // Calcular nuevas posiciones articulares usando cinemática inversa
...
class JointTrajectoryPublisher : public rclcpp::Node
    ...
    private:
        void publish_trajectory()
        {
            ...
            std::string config_path = get_file_path("ur_simulation_gz", "launch/config.txt");
